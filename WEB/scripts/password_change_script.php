...
if($isEmpty == true){
    header('Location: ../password_change.php?message=Nieco si nezadal');
}

if (!preg_match("/[0-9]/", $password) || !preg_match("/[0-9]/", $new_password)) {
    $hasPasswordAtLeastOneNumber = false;
    header('Location: ../password_change.php?message=Heslo musi obsahovat aspon jeden ciselny znak');
}

if($_POST["psw"] == $_POST["new-psw"]){
    $passwordAreSame = true;
    header('Location: ../password_change.php?message=Hesla su rovnake');
}
else{
    $passwordAreSame = false;
...
if(strlen($password) < 6){
    ...
    header('Location: ../password_change.php?message=Heslo musi obsahovat minimalne 6 znakov');
}
else{
    $hasPasswordCertainLength = true;
}
...
if($isEmpty == false && $hasPasswordCertainLength && $hasPasswordAtLeastOneNumber == true && $passwordAreSame == false){
    ...
    if ($conn->query($sql) == true){       
        header('Location: ../password_change.php?message=Zmena hesla bola uspesna!');
    }
    else{
